...
export const UsageQuestionStep = ({ stepLabel }: NumberedStepProps) => {
  ...
  if (!isStepActive) {
    const title = isStepCompleted
      ...
      : t`What will you use CloudVision for?`;
    return (
      <InactiveStep
        title={title}
        label={stepLabel}
  ...
  return (
    <ActiveStep title={t`What will you use CloudVision for?`} label={stepLabel}>
      <Radio.Group
        name="usage-reason"
        ...
        label={
          <Text
            color="text-light"
            fw="normal"
          >{t`Let us know your plans with CloudVision so that we can best guide you`}</Text>
        }
      >
        <Stack pt="lg">
          <Radio
